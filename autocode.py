...
lsum_train = gluon.data.DataLoader(
    ...
    batch_size=batch_size, shuffle=False, num_workers=4, last_batch='discard')
...
for epoch in range(nepoch):
    ...
    for data, _ in lsum_train:
        ...
        if iternum % 100 == 0:
            ...
            logging.info('errD_fake %f, errD_real %f, errG %f' % (
                mx.nd.mean(errD_fake).asscalar(), mx.nd.mean(errD_real).asscalar(), mx.nd.mean(errG).asscalar()))
            logging.info(
                'discriminator loss = %f, generator loss = %f, binary training acc = %f at iter %d epoch %d' % (
                    mx.nd.mean(-errD).asscalar(), mx.nd.mean(errG).asscalar(), acc, iternum, epoch))

        if iternum % 1000 == 999:
            visual('gout', fake.asnumpy(), name=os.path.join(outf, 'fake_img_iter_%d.png' % iternum))
            visual('data', data.asnumpy(), name=os.path.join(outf, 'real_img_iter_%d.png' % iternum))
            inception_score = get_inception_score_gl(netG, ctx)[0]
            real_score = get_real_inception_score(data)[0]
